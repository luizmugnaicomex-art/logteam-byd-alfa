...
import { UploadIcon, CloseIcon, AlertCircleIcon, Loader2Icon } from './common/Icons';

interface UploadModalProps {
    onCancel: () => void;
    onImport: (data: any[], onProgress: (progress: number) => void) => Promise<void>;
...
const UploadModal: React.FC<UploadModalProps> = ({ onCancel, onImport, parser, title }) => {
    ...
    const handleConfirmImport = async () => {
        ...
        setError(null);
        try {
            await onImport(parsedData, (p) => setProgress(p));
        } catch (err: any) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fade-in" onClick={isImporting ? undefined : onCancel}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl animate-scale-in" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b flex justify-between items-center"><h3 className="text-xl font-semibold">{title}</h3><button onClick={isImporting ? undefined : onCancel} className="text-gray-400 hover:text-gray-600 disabled:opacity-50"><CloseIcon /></button></div>
                <div className="p-6">
                    {!parsedData && (
                ...
                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse rounded-b-lg">
                    <button onClick={handleConfirmImport} disabled={!parsedData || !!error || isImporting} className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-byd-blue text-base font-medium text-white hover:bg-byd-blue/90 disabled:bg-gray-400 sm:ml-3 sm:w-auto sm:text-sm">
                        {isImporting ? <span className="flex items-center gap-2"><Loader2Icon /> {`Importing... ${progress}%`}</span> : 'Confirm Import'}
                    </button>
                    <button onClick={onCancel} disabled={isImporting} className="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50">Cancel</button>
                </div>
            </div>
        </div>
    );
