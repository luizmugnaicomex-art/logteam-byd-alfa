...
export const parseLogisticsFile = (jsonData: any[]): { data: LogisticsEntry[], error: string | null } => {
    try {
        ...
        for (const row of jsonData) {
            ...
            // Normalize: Uppercase and Trim to ensure uniqueness and prevent duplicates
            const normalizedCntr = String(cntrOriginalValue).trim().toUpperCase();
            const newEntry: Partial<LogisticsEntry> = { cntrsOriginal: normalizedCntr };
